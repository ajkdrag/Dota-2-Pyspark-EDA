...
def get_array_type_ohe_udf(mapping, col):
    """udf for one-hot-encoding an ArrayType column"""
...
def get_array_type_inv_ohe_udf(idx_mapping, val_mapping, col):
    """udf for inverting one-hot-encoded columns"""

    def invert_one_hot_encoded_array_type(col):
        return [
            [f"{idx_mapping.get(idx, 'None')}", f"{val_mapping.get(val, 'None')}"]
            for idx, val in enumerate(col)
        ]

    return udf(invert_one_hot_encoded_array_type, ArrayType(ArrayType(StringType())))(
        col
    )
