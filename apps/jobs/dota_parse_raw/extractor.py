...
class DotaRawExtractor(BaseExtractor):
    ...
    def _extract_matches(self, extra_columns=None):
        ...
        columns = ["winner", "cluster_id", "game_mode", "game_type"]
    ...
    def extract(self, entities):
        ...
        exploded_heroes_df = self._basic_explode(
            self._extract_heroes(), "heroes", only_exploded=True
        )
        exploded_lobbies_df = self._basic_explode(
            self._extract_lobbies(), "lobbies", only_exploded=True
        )
        exploded_regions_df = self._basic_explode(
            self._extract_regions(), "regions", only_exploded=True
        )

        id_name_df = exploded_heroes_df.sort("id").select("name")
