...
class DotaRawTransformer:
    ...
    def _transform_matches(self, matches_df):
        ...
        mapped_win_col_df = pk_added_matches_df.withColumn("winner", coalesce(mapping_rdd[col("winner")], lit("draw")))
        match_details_df = (
                            mapped_win_col_df
                            .select("match_id", "cluster_id", "game_mode", "game_type", "winner")
                            )
        ...
        merged_hero_ids_df = (
                                pk_added_matches_df
                                .withColumn("heroes", array(hero_id_ohe_cols))
    ...
    def _create_match_hero_xlkp_df(self, merged_hero_ids_df, ohe_heroes_df):
        ...
        hero_names_mapping = {idx: row[0] for idx, row in enumerate(id_name_df.collect())}
        hero_team_mapping = {"-1": "dire", "1": "radiant"}
        exploded_match_hero_names_df = (
                                        merged_hero_ids_df
                                        .withColumn("heroes", 
                                                    explode(get_array_type_inv_ohe_udf(hero_names_mapping, hero_team_mapping, "heroes"))
                                            )
                                        )
        filtered_match_hero_names_df = (
                                        exploded_match_hero_names_df
                                        .select("match_id", (col("heroes")[0]).alias("hero"), (col("heroes")[1]).alias("team"))
    ...
    def _lkp_match_lobby(self, match_details_df, lobbies_df):
        ...
        match_details_df = (
                            match_details_df.join(lkp_df, on=(match_details_df.game_mode==lkp_df.id), how="inner")
